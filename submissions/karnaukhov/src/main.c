...
static int bracket(func_t d, float *a, float *b) {
    ...
    for (float r = l + step; r <= 10.0f; r += step) {
        float cur = d(r);
        if (prev * cur <= 0) { *a = r - step; *b = r; return 0; }
        prev = cur;
    }
    return -1;
}
...
int main(int argc, char **argv) {
    ...
    float x12,x13,x23; int i12,i13,i23;
    ...
    if (!(l < m && m < r)) {
        ...
        for(int i=0;i<3;i++)for(int j=i+1;j<3;j++)if(arr[i]>arr[j]){float t=arr[i];arr[i]=arr[j];arr[j]=t;}
        l=arr[0]; m=arr[1]; r=arr[2];
    }
